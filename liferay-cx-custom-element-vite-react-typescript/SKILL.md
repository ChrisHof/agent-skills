...
Create a Liferay Custom Element Client Extension using _Vite_ as the build tool and _React & TypeScript_ from the template in the `/template` directory. Use the template as guidance for any further implementation of this extension.
...
3. Replace **_liferay-cx-custom-element_** in all files of the template with the given name of the client extension.
4. Run `yarn` or - if not available - `npm build` to install the dependencies.
